...
public class Game {
    ...
    public void start() {
        ...
        for (GameObject gameObject : gameObjects) {
            ...
            if (gameObject instanceof Destroyable) {
                ...
                while (!target.isDestroyable()) {

                    System.out.println("Making the shot..." + target);
    ...
    private GameObject[] createGameObjects(int numberObjects) {
        ...
        for (int i = 0; i < gameObjects.length; i++) {
            gameObjects[i] = Math.random() < ENEMY_PROBABILITY ? GameObjectFactory.createEnemy() : GameObjectFactory.createPissable();
        }
        return gameObjects;
